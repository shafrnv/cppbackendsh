...
struct Speed {
    ...
    // Конструктор с параметрами по умолчанию
    Speed(double vx_val = 0.0, double vy_val = 0.0) noexcept
        : vx(vx_val), vy(vy_val) {}
};
...
public:
    ...
    // Обновленный конструктор, включающий параметры для координат, скорости и направления
...
public:
    ...
    Dog& AddDog(std::string name,const Road& road);
private:
    Id id_;
