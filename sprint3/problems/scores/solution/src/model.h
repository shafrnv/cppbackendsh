...
public:
    ...
    void DeleteLostObjects(const LostObject::Id& id) noexcept{
        auto it = lost_object_id_to_index_.find(id);
        if (it != lost_object_id_to_index_.end()) {
            size_t index = it->second;
            std::cout << index << std::endl;

            lost_objects_.erase(lost_objects_.begin() + index);

            lost_object_id_to_index_.erase(it);

            for (auto& [key, idx] : lost_object_id_to_index_) {
                if (idx > index) {
                    --idx; 
                }
            }
